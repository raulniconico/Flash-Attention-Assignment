...
Answer to section 2.2 of the technical assignment (tasks 1–5).
...
| `flash_attention_pseudocode.py` | Task 1: control-CPU pseudocode of the optimised flash-attention kernel (prefill + decode), with the runtime API it assumes, numerical scheme, tiling, SRAM map and pipeline schedule in the header. Not executable by design. |
| `perf_model.py` | Tasks 2–5: analytical performance model built only from the constants of the assignment table. Produces every number quoted in the report (tile timing, elasticities, TTFT, interactivity, improvement variants, 4-accelerator analysis). |
| `model_output.txt` | Console log of `perf_model.py` — the numbers as generated. |
| `results.json` | Same results in machine-readable form. |
| `roofline.png` | Roofline of the accelerator with the four workload phases placed on it. |
...
| Flash-attention kernel | 1 365 MAC/cycle sustained (89 % of peak) in prefill; DRAM-bound stream on the vector CPU in decode |
| Bottleneck (task 2) | prefill: SRAM↔array port bandwidth, elasticity −1.0 (output ports −0.64); decode: DRAM path, −1.0 |
| TTFT (task 3) | ≈ 157 s for the 16 × 2048-token batch (compute-bound) |
| Interactivity (task 3) | ≈ 5.3 tok/s per sequence, 84.5 tok/s aggregate (189 ms/step, DRAM-bound) |
| Improvements (task 4) | ×4 memory bandwidth → 8.7 tok/s; INT4 weights + INT8 KV via DMA dequant → 8.7 tok/s; both → 10.3 tok/s |
| 4 accelerators (task 5) | TP = 4 for decode (DP for prefill): max 21 tok/s per sequence; ≥ 0.82 GB/s per accelerator link (3.3 GB/s aggregate) for 70 % |

## Key assumptions (full list with rationale in `report.md` §1.3)
